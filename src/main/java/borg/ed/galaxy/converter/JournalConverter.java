package borg.ed.galaxy.converter;
...
import borg.ed.galaxy.constants.StationType;
...
import borg.ed.galaxy.model.StarSystem.FactionPresence;
...
public class JournalConverter {
		...
		if (list == null || list.isEmpty()) {
			...
			for (Faction factionData : list) {
				...
				if (factionData.getRecoveringStates() == null) {
					...
					for (borg.ed.galaxy.journal.events.FSDJumpEvent.Faction.State stateData : factionData.getRecoveringStates()) {
				...
				if (factionData.getPendingStates() == null) {
					...
					for (borg.ed.galaxy.journal.events.FSDJumpEvent.Faction.State stateData : factionData.getPendingStates()) {
	...
	private List<Ring> ringsToRings(List<borg.ed.galaxy.journal.events.ScanEvent.Ring> list) {
		if (list == null || list.isEmpty()) {
			return null;
		} else {
			List<Ring> result = new ArrayList<>(list.size());
			for (borg.ed.galaxy.journal.events.ScanEvent.Ring ringData : list) {
				Ring ring = new Ring();
