...
import borg.ed.universe.repository.FsdJumpRepository;
import borg.ed.universe.repository.LocationRepository;
...
public class EddnElasticUpdater implements EddnUpdateListener {
    ...
    @Autowired
    private FsdJumpRepository fsdJumpRepository = null;
    ...
    void handleAbstractSystemJournalEvent(ZonedDateTime gatewayTimestamp, String uploaderID, AbstractSystemJournalEvent event) throws NonUniqueResultException {
        try {
            if (event instanceof FSDJumpEvent) {
                ...
                this.handleLocationEvent(uploaderID, (LocationEvent) event);
            }
    ...
    private void handleFsdJumpEvent(String uploaderID, FSDJumpEvent event) {
        try {
            FsdJump fsdJump = this.journalConverter.fsdJump(event);
            fsdJump.setUploaderId(uploaderID);
            fsdJump.setId(FsdJump.generateId(fsdJump));
            this.fsdJumpRepository.index(fsdJump);
        } catch (Exception e) {
            logger.error(e.getMessage());
        }
    }

    private void handleLocationEvent(String uploaderID, LocationEvent event) {
        try {
            Location location = this.journalConverter.location(event);
            location.setUploaderId(uploaderID);
            location.setId(Location.generateId(location));
            this.locationRepository.index(location);
        } catch (Exception e) {
            logger.error(e.getMessage());
        }
    }
