		...
		if (existingBody == null) {
			...
			if (logger.isTraceEnabled()) {
				if (currentBody.getStarClass() != null) {
					logger.trace(String.format(Locale.US, "New class %s star discovered by %s: '%s' %,.1f Ly from Sol", currentBody.getStarClass(), uploaderID, currentBody.getName(),
							currentBody.getCoord().distanceTo(new Coord())));
				} else {
					logger.trace(String.format(Locale.US, "New %s discovered by %s: '%s' %,.1f Ly from Sol", currentBody.getPlanetClass(), uploaderID, currentBody.getName(),
							currentBody.getCoord().distanceTo(new Coord())));
				}
			}
