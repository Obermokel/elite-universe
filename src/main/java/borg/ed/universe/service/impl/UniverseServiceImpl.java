...
public class UniverseServiceImpl implements UniverseService {
    ...
    public StarSystem findStarSystemByName(String name) throws NonUniqueResultException {
        ...
        if (page.getTotalElements() < 1) {
            ...
            throw new NonUniqueResultException("Found " + page.getTotalElements() + " systems for '" + name + "'", page.getContent().stream().map(StarSystem::toString).collect(Collectors.toList()));
        } else {
    ...
    public MinorFaction findMinorFactionByName(String name) throws NonUniqueResultException {
        ...
        if (page.getTotalElements() < 1) {
            ...
            throw new NonUniqueResultException("Found " + page.getTotalElements() + " minor factions for '" + name + "'", page.getContent().stream().map(MinorFaction::toString).collect(Collectors.toList()));
        } else {
            return page.getContent().get(0);
        }
    }
